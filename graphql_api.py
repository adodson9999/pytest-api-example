...
def _ensure_inventory_for_pet(pet_id: int):
    ...
    if existing is None:
        inventory_data.append(
            {"id": _next_id(inventory_data), "inventory": 1, "pet_id": pet_id}
        )
...
def _fault(info) -> str | None:
    ctx = getattr(info, "context", None)
    if isinstance(ctx, dict):
        return ctx.get("fault_mode")
    return None
...
def resolve_pets(_, info, status=None, type=None):
    if _fault(info) == "resolver_exception_pets":
        # MUST raise GraphQLError or you'll sometimes get odd shapes
        raise GraphQLError("Synthetic resolver crash (pets)")
...
def resolve_inventory(_, info, pet_id=None):
    # ✅ this is what your chaos test expects
    if _fault(info) == "downstream_inventory_failure":
        raise GraphQLError("Downstream inventory domain unavailable")
...
# ----------------------------
# Field-level chaos: Pet.status partial failure
# ----------------------------
pet_obj = ObjectType("Pet")

@pet_obj.field("status")
def resolve_pet_status(pet, info):
    if _fault(info) == "partial_pet_status_failure":
        # fail for some records so you get partial data + errors
        if int(pet.get("id", 0)) % 2 == 0:
            raise GraphQLError("Pet.status unavailable for this record")
    return pet.get("status")
...
def resolve_create_pet(_, info, name, type, status="available"):
    ...
    # chaos: inventory sync failure should error AND preserve integrity
    if _fault(info) == "inventory_sync_fail":
        pets_data.pop()  # rollback
        raise GraphQLError("Inventory sync failed for created pet")
...
def resolve_update_vendor(_, info, id, name=None, contact_form=None, contact_info=None, point_of_contact=None, product=None):
...
# Executable schema
# ----------------------------
schema = make_executable_schema(type_defs, query, mutation, pet_obj)


# ----------------------------
# init_graphql: observability + chaos wiring
# ----------------------------
def init_graphql(app, pets_store, orders_store, events, inventory, vendors, trainers, customers, vets):
    """
    Observability:
      - X-Request-Id correlation
      - JSON log lines: graphql_request, graphql_error, graphql_request_invalid
      - File logging in testing mode (GRAPHQL_LOG_PATH)

    Chaos:
      - X-Fault enabled only in testing
      - passed into context as context["fault_mode"]
    """
    ...
    testing_enabled = (env == "testing") or bool(app.config.get("TESTING", False))
    ...
    if testing_enabled:
        fh = RotatingFileHandler(str(log_path), maxBytes=2_000_000, backupCount=2, encoding="utf-8")
    ...
    logger.info(json.dumps({
        ...
        "log_path": str(log_path) if testing_enabled else None,
    }))
    ...
    def _get_fault_mode() -> str | None:
        if not testing_enabled:
            return None
        return request.headers.get("X-Fault")
    ...
    def graphql_server():
        ...
        context = {"request": request, "request_id": request_id, "fault_mode": fault_mode}
